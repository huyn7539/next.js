...
pub enum TtlCounter {
    ...
    ///
    /// Only a first-of-session pass writes this, so the stamp records the instant of that sample
    /// rather than the moment the root actually went idle, and it is never rewritten while the
    /// root stays stale. That coarseness is intended: see the backend's GC root TTL
    /// (`DEFAULT_GC_ROOT_TTL`), which is a backstop for pins that outlive their holder rather
    /// than the primary way roots are reclaimed.
    FirstStale(u64),
}
...
pub(crate) struct GcStats {
    ...
    ///
    /// Only a task that was **not** resident when the pass began can land here: every resident
    /// entry, and every aged-out one, is already dropped by
    /// [`TurboTasksBackend::gc_roots_refresh_and_age_out`] before the collection phase starts. So
    /// this captures just the rare case where a concurrent cascade pulled a non-resident root
    /// into residency and collected it.
