...
Each key identifies one root. `path` can be absolute or relative to the directory where you run Next.js. `optional` defaults to `false`.

Next.js canonicalizes each path before validating it. Missing required roots and overlapping roots produce configuration errors and are excluded. Missing optional roots are silently excluded. Roots cannot overlap `turbopack.root` or one another; when multiple entries overlap, the first entry in JavaScript property order is used.
