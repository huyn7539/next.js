...
| `additionalRoots`   | Allows symlinked dependencies to resolve into explicitly configured filesystem roots.                                                    |
...
For symlinked dependencies, you can instead keep the project root narrow and configure the linked locations with `additionalRoots`.

### Additional roots

`additionalRoots` allows Turbopack to follow symbolic links whose targets are outside `turbopack.root`. It is useful for `npm link` and package-manager stores that live outside the project, including pnpm's Global Virtual Store.
...
module.exports = {
  turbopack: {
    additionalRoots: {
      ...
      pnpmStore: {
        path: path.join(os.homedir(), '.local/share/pnpm/store'),
...
Each key identifies one root. `path` can be absolute or relative to the directory where you run Next.js. `ignoreIfMissing` defaults to `false`.

Next.js canonicalizes each path before validating it. Missing required roots and overlapping roots produce configuration errors and are excluded. Missing roots with `ignoreIfMissing: true` are silently excluded. Roots cannot overlap `turbopack.root` or one another; when multiple entries overlap, the first entry in JavaScript property order is used.

Additional roots are read-only and watched non-recursively. They only permit symlink crossings: direct relative imports that escape the project root remain unsupported.
